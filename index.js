...
var gradientPercent = function(d, candidate) {
  var total = parseInt(d.value);
  var word = d.name;
  var share;
  if (candidate == "sanders") {
    share = data.Total.Democrat.Sanders[word];
  }
  else if (candidate == "clinton") {
    share = data.Total.Democrat.Clinton[word];
  }
  else if (candidate == "trump") {
    share = data.Total.Republican.Trump[word];
  }
  else if (candidate == "kasich") {
    share = data.Total.Republican.Kasich[word];
  }
  else if (candidate == "cruz") {
    share = data.Total.Republican.Cruz[word];
  }
  console.log(data.Total.Democrat.Sanders[word]);
  return parseInt(share) / total * 100;
};
...
//Add mousehover interactivity
      ...
      .on("click", function(d) {
        ...
        d3.select("#value").text("Total: " + d.value);
        d3.select("#sanders").text(": " + data.Total.Democrat.Sanders[d.name]);
        d3.select("#clinton").text(": " + data.Total.Democrat.Clinton[d.name]);
        d3.select("#trump").text(": " + data.Total.Republican.Trump[d.name]);
        d3.select("#kasich").text(": " + data.Total.Republican.Kasich[d.name]);
        d3.select("#cruz").text(": " + data.Total.Republican.Cruz[d.name]);
      })
      ...
      .attr("fill", function(d){
        ...
        var currPercent = 0;
    	  ...
    	  gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", sanders);
        currPercent += gradientPercent(d, "sanders");
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", sanders);
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", clinton);
        currPercent += gradientPercent(d, "clinton");
    	  gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", clinton);
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", trump);
        currPercent += gradientPercent(d, "trump");
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", trump);
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", kasich);
        currPercent += gradientPercent(d, "kasich");
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", kasich);
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", cruz);
        currPercent += gradientPercent(d, "cruz");
        gradient.append("stop")
    	      .attr("offset", currPercent+"%")
    	      .attr("stop-color", cruz);
